...
import { Clock3, Eye, EyeOff, Image as ImageIcon, Loader2, Pencil, Plus, Search, UtensilsCrossed, X } from 'lucide-react';
...
export const MenuView = () => {
    ...
    const [form, setForm] = useState({ name: '', description: '', categoryId: '', price: '', prepTimeMinutes: '0', imageUrl: '', isAvailable: true });
    ...
    const resetForm = () => {
        setEditing(null);
        setForm({ name: '', description: '', categoryId: categories[0]?.id || '', price: '', prepTimeMinutes: '0', imageUrl: '', isAvailable: true });
    };
    const beginEdit = (item: RestaurantMenuItem) => {
        setEditing(item);
        setForm({ name: item.name, description: item.description || '', categoryId: item.categoryId, price: (item.price / 100).toFixed(2), prepTimeMinutes: String(item.prepTimeMinutes), imageUrl: item.imageUrl || '', isAvailable: item.isAvailable });
        window.scrollTo({ top: 0, behavior: 'smooth' });
    };

    const createCategory = async (event: FormEvent) => {
        event.preventDefault(); const name = categoryName.trim(); if (!name || saving) return; setSaving(true);
        try { const category = await restaurantApi.createMenuCategory({ name }); setCategoryName(''); setSelectedCategory(category.id); await load(); setNotice({ kind: 'success', text: `${category.name} category created.` }); }
    ...
    const saveItem = async (event: FormEvent) => {
        ...
        setSaving(true);
        try {
            if (editing) await restaurantApi.updateMenuItem(editing.id, { name: form.name.trim(), description: form.description, categoryId: form.categoryId, price, prepTimeMinutes, imageUrl: form.imageUrl, isAvailable: form.isAvailable });
            else { const created = await restaurantApi.createMenuItem({ name: form.name.trim(), category_id: form.categoryId, price, description: form.description, prep_time_minutes: prepTimeMinutes }); if (form.imageUrl || !form.isAvailable) await restaurantApi.updateMenuItem(created.id, { imageUrl: form.imageUrl, isAvailable: form.isAvailable }); }
            setNotice({ kind: 'success', text: `${form.name.trim()} ${editing ? 'updated' : 'added to the menu'}.` }); resetForm(); await load();
        } catch { setNotice({ kind: 'error', text: 'Unable to save this menu item.' }); } finally { setSaving(false); }
    };
    ...
    return <div className="menu-manager">
        ...
        <div className="menu-workspace">
            <aside className="menu-sidebar"><div className="menu-panel-title"><div><strong>Categories</strong><small>Organize the customer menu</small></div></div>
                ...
                <form className="category-form" onSubmit={createCategory}><input value={categoryName} onChange={event => setCategoryName(event.target.value)} placeholder="New category" maxLength={80} /><button type="submit" disabled={!categoryName.trim() || saving}><Plus size={16} /></button></form>
            </aside>
            <main className="menu-main"><div className="menu-toolbar"><label><Search size={18} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search items or descriptions" /></label><button type="button" onClick={resetForm}><Plus size={17} /> New item</button></div>
            ...
            <form className="item-editor" onSubmit={saveItem}><div className="menu-panel-title"><div><strong>{editing ? 'Edit menu item' : 'New menu item'}</strong><small>{editing ? 'Changes update POS and public menu' : 'Add an item used by POS'}</small></div>{editing && <button type="button" onClick={resetForm}><X size={17} /></button>}</div>
                <label>Item name<input value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} maxLength={120} placeholder="Chicken sandwich" /></label><label>Description<textarea value={form.description} onChange={event => setForm({ ...form, description: event.target.value })} rows={3} placeholder="Short customer-friendly description" /></label><label>Category<select value={form.categoryId} onChange={event => setForm({ ...form, categoryId: event.target.value })}><option value="">Choose category</option>{categories.map(category => <option value={category.id} key={category.id}>{category.name}</option>)}</select></label>
                <div className="editor-split"><label>Price<input type="number" min="0.01" step="0.01" value={form.price} onChange={event => setForm({ ...form, price: event.target.value })} placeholder="0.00" /></label><label>Prep minutes<input type="number" min="0" max="480" step="1" value={form.prepTimeMinutes} onChange={event => setForm({ ...form, prepTimeMinutes: event.target.value })} /></label></div><label>Image URL <small>(optional HTTPS)</small><input type="url" value={form.imageUrl} onChange={event => setForm({ ...form, imageUrl: event.target.value })} placeholder="https://…" /></label><label className="availability-toggle"><input type="checkbox" checked={form.isAvailable} onChange={event => setForm({ ...form, isAvailable: event.target.checked })} /><span><strong>Available for sale</strong><small>Visible on POS and customer QR menu</small></span></label><button className="save-item" type="submit" disabled={saving || !categories.length}>{saving ? <Loader2 size={17} /> : <Plus size={17} />}{saving ? 'Saving…' : editing ? 'Save changes' : 'Add menu item'}</button>{!categories.length && <small className="editor-help">Create a category before adding an item.</small>}
            </form>
        </div>
        <style>{`
