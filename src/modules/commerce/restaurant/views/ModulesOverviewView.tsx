...
import { BarChart3, ChefHat, ContactRound, FileText, LayoutDashboard, LayoutGrid, Loader2, LockKeyhole, Package, Receipt, Search, ShieldCheck, ShoppingCart, Users, UtensilsCrossed, WalletCards } from 'lucide-react';
...
const ICONS: Record<string, typeof LayoutGrid> = {
    ...
    finance: WalletCards, customers: ContactRound, quotes: FileText,
};
...
export const ModulesOverviewView = () => {
    ...
    // CORE-M1: shared modules render once in their own section; workspace
    // modules stay in the workspace section. Display context only — the
    // backend remains the authorization authority.
    const sharedModules = useMemo(() => visible.filter(module => module.workspaceKey === 'shared'), [visible]);
    const workspaceModules = useMemo(() => visible.filter(module => module.workspaceKey !== 'shared'), [visible]);
    ...
    const toggle = async (module: BusinessModule) => {
        ...
        try {
            ...
            setMessage({ kind: 'success', text: module.enabled ? `${module.label} disabled and removed from navigation. Saved data was not deleted.` : `${module.label} enabled for this business.` });
        } catch (error) {
    ...
    return (
        <ModuleShell padding="clamp(18px,4vw,32px)">
            <div className="module-drawer">
                <PageHeader title="Add Modules" subtitle="Manage shared business modules and the tools shown in this Restaurant workspace." />
                ...
                {loading && !modules.length ? <div className="module-drawer-loading"><Loader2 size={20} /> Loading modules...</div> : (
                    <>
                        {sharedModules.length > 0 && (
                            <section className="module-drawer-section">
                                <h2 className="module-drawer-section-title">Shared Modules</h2>
                                <p className="module-drawer-section-sub">Tenant-wide modules shared by every workspace in this business.</p>
                                <div className="module-drawer-grid">{sharedModules.map(module => <ModuleCard key={`shared:${module.moduleKey}`} module={module} busy={busy === module.moduleKey} onToggle={toggle} />)}</div>
                            </section>
                        )}
                        {workspaceModules.length > 0 && (
                            <section className="module-drawer-section">
                                <h2 className="module-drawer-section-title">Workspace Modules</h2>
                                <p className="module-drawer-section-sub">Modules scoped to this Restaurant workspace.</p>
                                <div className="module-drawer-grid">{workspaceModules.map(module => <ModuleCard key={`workspace:${module.moduleKey}`} module={module} busy={busy === module.moduleKey} onToggle={toggle} />)}</div>
                            </section>
                        )}
                    </>
                )}
