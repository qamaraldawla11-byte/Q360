...
import { restaurantApi, type RestaurantQDraft, type RestaurantQPulse, type RestaurantQUsage } from '@/api/restaurant.api';
...
export const AssistantView = () => {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        try {
            const [pulseResult, draftResult, usageResult] = await Promise.all([
                restaurantApi.getBusinessPulse(),
                restaurantApi.getQDrafts(),
                restaurantApi.getQUsage().catch(() => null),
            ]);
    ...
    return <div className="q-assistant">
        ...
        {loading ? <div className="q-loading">Q is building a tenant-safe Restaurant Pulse...</div> : pulse && <>
            {usage && <section className="q-usage"><div><small>THIS MONTH</small><b>{usage.requests}</b><span>Q requests</span></div><div><small>COMPLETED</small><b>{usage.completed}</b><span>safe responses</span></div><div><small>MODEL COST</small><b>${usage.estimatedCostUsd.toFixed(2)}</b><span>rules-only Q is currently $0</span></div></section>}
        ...
        `}</style><style>{`.q-usage{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-bottom:13px}.q-usage>div{padding:14px;border:1px solid #293444;border-radius:13px;background:#101722}.q-usage small,.q-usage span{display:block;color:#94a3b8;font-size:11px}.q-usage b{display:block;margin:5px 0;color:#e0f2fe;font-size:25px}@media(max-width:760px){.q-usage{grid-template-columns:1fr}}`}</style>
    </div>;
};
