...
const operatingPriorities: Array<{ key: RestaurantQOperatingPriority; label: string }> = [
    { key: 'service_speed', label: 'Service speed' }, { key: 'guest_experience', label: 'Guest experience' },
    { key: 'cost_control', label: 'Cost control' }, { key: 'sales_growth', label: 'Sales growth' },
    { key: 'waste_reduction', label: 'Waste reduction' }, { key: 'team_development', label: 'Team development' },
];
...
export const AssistantView = () => {
    ...
    const saveMemory = async () => {
        setBusy('memory'); setNotice('');
        try {
            const result = await restaurantApi.updateQBusinessMemory({ ownerSummary: memory.ownerSummary, businessGoals: memory.businessGoals, operatingPriorities: memory.operatingPriorities });
            setMemory(result.memory); setNotice(result.message); setError('');
        } catch (reason) { setError(reason instanceof Error ? reason.message : 'Q business context could not be saved.'); }
        finally { setBusy(''); }
    };

    const togglePriority = (priority: RestaurantQOperatingPriority) => setMemory(current => ({
        ...current,
        operatingPriorities: current.operatingPriorities.includes(priority) ? current.operatingPriorities.filter(item => item !== priority) : [...current.operatingPriorities, priority],
    }));
    ...
    return <div className="q-assistant">
        ...
        <details className="q-memory"><summary><span><Bot size={17} /> How Q should help this business</span><small>Owner and admin only</small></summary><p>Save a short operating context. Q uses it only with this restaurant&apos;s records; it does not give Q permission to make changes.</p><div className="q-memory-fields"><label>About this restaurant<textarea value={memory.ownerSummary ?? ''} maxLength={1200} onChange={event => setMemory(current => ({ ...current, ownerSummary: event.target.value || null }))} placeholder="e.g. Family cafe with weekday takeaway and weekend dine-in." /></label><label>Current business goal<textarea value={memory.businessGoals ?? ''} maxLength={1200} onChange={event => setMemory(current => ({ ...current, businessGoals: event.target.value || null }))} placeholder="e.g. Improve lunchtime speed while controlling food costs." /></label></div><div className="q-priorities">{operatingPriorities.map(priority => <label key={priority.key}><input type="checkbox" checked={memory.operatingPriorities.includes(priority.key)} onChange={() => togglePriority(priority.key)} /> {priority.label}</label>)}</div><button type="button" className="q-memory-save" disabled={busy === 'memory'} onClick={() => void saveMemory()}>{busy === 'memory' ? 'Saving...' : 'Save Q business context'}</button></details>
